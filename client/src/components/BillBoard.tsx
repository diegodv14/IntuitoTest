...
export const Cartelera: React.FC<CarteleraProps> = ({ BillBoard }) => {
    ...
    return (
        <li key={BillBoard.id} className="size-36 relative w-full h-[200px] rounded-lg flex flex-col justify-between">
            <div className={`rounded-md w-full relative h-[80%] shadow-md ${BillBoard.id === 1 ? "imageMovie1" : "imageMovie2"}`} onMouseOver={() => setIsEditing(true)} onMouseOut={() => setIsEditing(false)}>
                {IsEditing && <>
