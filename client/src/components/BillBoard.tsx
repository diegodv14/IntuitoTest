...
export const Cartelera: React.FC<CarteleraProps> = ({ BillBoard }) => {
    ...
    const DeleteBillBoard = async (id: number) => {
        ...
        if (!response) {
            setErrorMessage('No se puede eliminar una cartelera con fecha anterior a la actual')
            setTimeout(() => {
                setErrorMessage("")
            }, 3000)
        } else {
    ...
    return (
        <>
            <li key={BillBoard.id} className={`size-36 ${errorMessage && 'grayscale'} relative w-full h-[200px] rounded-lg flex flex-col justify-between`}>

                <div className={`rounded-md w-full relative h-[80%] shadow-md ${BillBoard.id === 1
                    ? 'imageMovie1'
                    : BillBoard.id === 2
                        ? 'imageMovie2'
                        : BillBoard.id === 3 ? 'imageMovie3' : 'imageDefault'
                    }`} onMouseOver={() => setIsEditing(true)} onMouseOut={() => setIsEditing(false)}>
                    {IsEditing && <>
                        ...
                        <span className="absolute bottom-2 right-2 text-white font-[Tanker]">{String(BillBoard.date)}</span>
                    </>}
                </div>
            ...
            {errorMessage && <p className="absolute text-red-600 top-8 left-80 font-semibold z-50 text-[12px]">{errorMessage}</p>}
        </>
    )
}